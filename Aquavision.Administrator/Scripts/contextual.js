    ...
    button(label, onClick, shortcut = '', icon = '', cssIcon = '', enabled = true){
        this.element = contextualCore.CreateEl( `
            <li class='contextualJs contextualMenuItemOuter'>
                <div class='contextualJs contextualMenuItem ${enabled ? '' : 'disabled'}'>
    ...
    hoverMenu(label, items, icon = '', cssIcon = '', enabled = true){
        this.element = contextualCore.CreateEl(`
            <li class='contextualJs contextualHoverMenuOuter'>
                <div class='contextualJs contextualHoverMenuItem ${enabled ? '' : 'disabled'}'>
                    ${icon != ''? `<img src='${icon}' class='contextualJs contextualMenuItemIcon'/>` : `<div class='contextualJs contextualMenuItemIcon ${cssIcon != '' ? cssIcon : ''}'></div>`}
                    <span class='contextualJs contextualMenuItemTitle'>${label == undefined ? 'No label' : label}</span>
...
const contextualCore = {
    PositionMenu: (docked, el, menu) => {
        if (docked) {
            menu.style.left = ((el.target.offsetLeft + menu.offsetWidth) >= window.innerWidth) ? ((el.target.offsetLeft - menu.offsetWidth) + el.target.offsetWidth)+'px' : (el.target.offsetLeft)+'px';
            menu.style.top = ((el.target.offsetTop + menu.offsetHeight) >= window.innerHeight) ? (el.target.offsetTop - menu.offsetHeight)+'px' : (el.target.offsetHeight + el.target.offsetTop)+'px';
        } else {
            menu.style.left = ((el.pageX + menu.offsetWidth) >= window.innerWidth) ? ((el.pageX - menu.offsetWidth))+'px' : (el.pageX)+'px';
            menu.style.top = ((el.pageY + menu.offsetHeight) >= window.innerHeight) ? (el.pageY - menu.offsetHeight)+'px' : (el.pageY)+'px';
        }
    },
    CloseMenu: () => {
